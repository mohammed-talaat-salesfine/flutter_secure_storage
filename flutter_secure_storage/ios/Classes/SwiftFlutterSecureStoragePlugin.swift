...
public class SwiftFlutterSecureStoragePlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    private func read(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        if(values.laRead){
            laContext = LAContext()
            laContext?.localizedReason = values.reason ?? ""
            laContext?.touchIDAuthenticationAllowableReuseDuration = 3
        }
        
        let response = flutterSecureStorageManager.read(key: values.key!, groupId: values.groupId, accountName: values.accountName,laContext: laContext)
    ...
    private func write(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        if(values.laRead){
            ...
            context.localizedReason = values.reason ?? ""
            context.touchIDAuthenticationAllowableReuseDuration = 3
                      
            let response = flutterSecureStorageManager.writeWithLocalAuth(key: values.key!, value: values.value!, laContext: context, groupId: values.groupId, accountName: values.accountName,synchronizable: values.synchronizable)
    ...
    private func parseCall(_ call: FlutterMethodCall) -> FlutterSecureStorageRequest {
        ...
        let reason = options["LaReason"] as? String
        let accessibility = options["accessibility"] as? String
        let value = arguments["value"] as? String
        let laReadString = options["laRead"] as? String
        
        let laRead: Bool = laReadString != nil ? Bool(laReadString!)! : false
